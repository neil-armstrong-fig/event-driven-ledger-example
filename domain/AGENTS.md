...
the root `AGENTS.md` package table. **No `src/` yet** — this is the very next package to get real
code, at Phase 3 (see `TODO.md`). Don't invent structure ahead of the first failing test; TDD here
means the test picks the shape, not the other way round.
...
concerns (Phase 4/5) — this package proves only the pure shape the rest of the system relies on.
